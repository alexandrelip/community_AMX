...
try {
    for($cycle=1;$cycle -le $Cycles;$cycle++){
        for($index=0;$index -lt $cases.Count;$index++){
            $case=$cases[$index]
            $stepName=('keyboard-core-{0:D2}-{1:D3}-{2}.json' -f $cycle,($index+1),$case.Control)
    ...
    Write-IntegrationJson $reportPath ([ordered]@{Schema='AMXDENIS_KEYBOARD_CORE_1';BuildId=$state.BuildId;
        ...
        Scope='Selected core producer transitions, not all catalog functions, rendered pixels or mechanical actuation';
