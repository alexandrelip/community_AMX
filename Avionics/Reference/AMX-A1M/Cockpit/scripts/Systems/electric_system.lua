...
local native_power = dofile(LockOn_Options.script_path .. "../../Config/AMX_EDM_BINDINGS.lua").enabled == true
local native_link = get_param_handle("AMX_ELEC_NATIVE_LINK")
local failed_battery = get_param_handle("AMX_FAIL_BATTERY")
local failed_left = get_param_handle("AMX_FAIL_GEN_L")
local failed_right = get_param_handle("AMX_FAIL_GEN_R")
local battery_available_param = get_param_handle("AMX_ELEC_BATTERY_AVAILABLE")
local left_available_param = get_param_handle("AMX_ELEC_GEN_L_AVAILABLE")
local right_available_param = get_param_handle("AMX_ELEC_GEN_R_AVAILABLE")
...
function update()
	...
	if T_GENEG:get() == 1 and engine_rpm > 53 and failed_left:get() < 0.5 then
		GENEGIsOn = 1
		else
		GENEGIsOn = 0
	end
	-- Switch Right Generator
	if T_GENED:get() == 1 and engine_rpm > 53 and failed_right:get() < 0.5 then
		GENEDIsOn = 1
		else
		GENEDIsOn = 0
	end
	-- Main bus 
	local battery_available = MasterBattIsOn == 1 and failed_battery:get() < 0.5
	battery_available_param:set(battery_available and 1 or 0)
	left_available_param:set(GENEGIsOn)
	right_available_param:set(GENEDIsOn)
	if battery_available or GENEDIsOn == 1 or GENEGIsOn == 1 then
		ELEC_P1:set(1)
		ELEC_P2:set(1)
		else
		ELEC_P1:set(0)
	...
	ELEC_P3:set(ELEC_P1:get() == 1 and T_DELESTAGE:get() == 1 and 1 or 0)
	local linked = native_power and type(dev.DC_Battery_on) == "function"
		and type(dev.AC_Generator_1_on) == "function" and type(dev.AC_Generator_2_on) == "function"
	if linked then
		dev:DC_Battery_on(battery_available)
		dev:AC_Generator_1_on(GENEGIsOn == 1)
		dev:AC_Generator_2_on(GENEDIsOn == 1)
	end
	native_link:set(linked and 1 or 0)
