...
-- [F-5EM 2026-07-13] addDcltGate helper (restaurado apos revert HUD v0.71).
-- O revert `fbcb7af` (2026-07-13 13:10) restaurou HUD_ALL/HUD_AG/hud.lua/HUD_defs
-- para o baseline v0.71 (ICP/HUD estavam quebrados na cascade v0.72-v0.81), mas
-- test_hud_declutter_gates.lua continua no CI e depende dessa funcao. HUD_ALL
-- v0.71 NAO chama addDcltGate atualmente -- e helper reservado para o dia em
-- que declutter multi-level (NORM/REJ1/REJ2) for reintroduzido de forma
-- estavel. Guards `type() ~= "table"` sao defensivos contra `object.element_params
-- = ""` que CreateElement retorna quando o field nao foi explicitamente setado
-- (bug historico de 2026-07-12: table.insert crashava com "table expected, got
-- string"). Impl identica a ci/tests/test_hud_declutter_gates.lua L36-52.
function addDcltGate(object, hideLevel, params, ctrls)
    if object == nil or hideLevel == nil then return end
    if type(params) ~= "table" then params = {} end
    local paramIdx = #params
    table.insert(params, "HUD_DCLT")

    if type(ctrls) ~= "table" then
        ctrls = {}
    elseif #ctrls > 0 and type(ctrls[1]) == "string" then
        ctrls = {ctrls}
    end
    table.insert(ctrls, {"parameter_in_range", paramIdx, -0.05, hideLevel - 0.5})
	object.element_params = params
    object.controllers = ctrls
end
...
function create_page_root()
	...
	page_root.controllers = {
		{"parameter_compare_with_number",0,1},
		{"opacity_using_parameter", 1},
		{"scale", HUD_LAYOUT_SCALE, HUD_LAYOUT_SCALE, 1},
	}
	default_parent = page_root.name
    return page_root
end

function create_hud_scaled_root(name)
	local root = addPlaceholder(name, {0, 0})
	root.controllers = {{"scale", HUD_LAYOUT_SCALE, HUD_LAYOUT_SCALE, 1}}
	return root
end
...
fontIntercharDflt_100	= 1.2
fontInterlineDflt_100	= 1.5
...
fontIntercharDflt_75	= 1.0
fontInterlineDflt_75	= 1.5
...
fontIntercharDflt_120	= 1.2
fontInterlineDflt_120	= 1.5
...
fontIntercharDflt_60	= 1.0
fontInterlineDflt_60	= 1.5
