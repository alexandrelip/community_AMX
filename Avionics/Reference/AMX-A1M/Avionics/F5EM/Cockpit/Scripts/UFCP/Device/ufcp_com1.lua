...
function ufcp_com1_check()
    if radio == nil then radio = GetDevice(devices.UHF_RADIO) end
    if type(radio) == 'number' then radio = nil end

    if radio then
        local freq = radio:get_frequency()
        if math.floor(freq/1e3) ~= (math.floor(ufcp_com1_memory.frequency_last * 1e3)) then
            ufcp_com1_frequency = freq / 1e6
            ufcp_com1_frequency_manual = freq / 1e6
            ufcp_com1_frequency_sel = UFCP_COM_FREQUENCY_SEL_IDS.MAN
        end
    end

    if ufcp_com1_memory.sql_last ~= ufcp_com1_sql then
        if radio then
            radio:performClickableAction(device_commands.Button_10, ufcp_com1_sql and 1 or 0)
        end
        ufcp_com1_memory.sql_last = ufcp_com1_sql
    end

    if ufcp_com1_memory.frequency_last ~= ufcp_com1_frequency then
        if radio then
            radio:performClickableAction(iCommandPlaneUHFFreqModeDialMNL, 1)
            radio:performClickableAction(iCommandPlaneUHFFunctionDialMAIN, 1)
    ...
    if ufcp_com1_memory.modulation_last ~= ufcp_com1_modulation then
        if radio then
            if ufcp_com1_modulation == UFCP_COM_MODE_IDS.AM then
                radio:set_modulation(UFCP_COM_MODULATION_IDS.AM)
            elseif ufcp_com1_modulation == UFCP_COM_MODE_IDS.FM then
                radio:set_modulation(UFCP_COM_MODULATION_IDS.FM)
            end
        end
...
-- [DTC] UHF preset frequencies published by the DTU page (consumed on DTC load)
local DTC_COMM1_COUNT     = get_param_handle("DTC_COMM1_COUNT")
local CMFD_DTU_DTC_LOADED = get_param_handle("CMFD_DTU_DTC_LOADED")
local DTC_COMM1_FREQ = {}
for i = 1, 20 do
    DTC_COMM1_FREQ[i] = get_param_handle(string.format("DTC_COMM1_%02d_FREQ", i))
end
local com1_dtc_loaded_prev = 0

-- [DTC] On the rising edge of a DTC load, copy the DTC's UHF preset
-- frequencies (DTC_COMM1_NN_FREQ, 225-399.975 MHz) into the COM1 preset
-- channels: preset i -> channel i. An empty/out-of-band slot is skipped so
-- it keeps whatever the pilot had; the live tuned frequency is not changed
-- (the pilot selects a channel to use a preset, as on the real jet).
function update_com1_dtc()
    local loaded = CMFD_DTU_DTC_LOADED:get()
    if loaded > 0.5 and com1_dtc_loaded_prev <= 0.5 then
        if DTC_COMM1_COUNT:get() > 0 then
            for i = 1, 20 do
                local f = DTC_COMM1_FREQ[i]:get()
                if f >= 225 and f <= 399.975 and (i + 1) <= (ufcp_com1_max_channel + 1) then
                    ufcp_com1_channels[i + 1] = f
                end
            end
        end
    end
    com1_dtc_loaded_prev = loaded
end
