local date_validation = dofile(LockOn_Options.script_path.."UFCP/Device/date_validation.lua")
...
-- [Fase R3.9] COMSEC — publish current ECCM selector so Systems/comsec.lua
-- can react (garble / key-loaded / TRANSEC hop). Values match UFCP_COM2_ECCM_IDS:
--   1=PLAIN, 2=COMSEC, 3=TRANSEC, 4=HAILING
UFCP_COM2_ECCM = get_param_handle("UFCP_COM2_ECCM")
...
function ufcp_com2_check()
    if radio == nil then radio = GetDevice(devices.UHF_RADIO1) end
    if type(radio) == 'number' then radio = nil end

    if radio then
        local freq = radio:get_frequency()
        if math.floor(freq/1e3) ~= (math.floor(ufcp_com2_memory.frequency_last * 1e3)) then
            ufcp_com2_frequency = freq / 1e6
            ufcp_com2_frequency_manual = freq / 1e6
            ufcp_com2_frequency_sel = UFCP_COM_FREQUENCY_SEL_IDS.MAN
        end
    end

    if ufcp_com2_memory.sql_last ~= ufcp_com2_sql then
        if radio then
            radio:performClickableAction(device_commands.Button_10, ufcp_com2_sql and 1 or 0)
        end
        ufcp_com2_memory.sql_last = ufcp_com2_sql
    end
    ...
    if ufcp_com2_memory.modulation_last ~= ufcp_com2_modulation then
        if radio then
            if ufcp_com2_modulation == UFCP_COM_MODE_IDS.AM then
                radio:set_modulation(UFCP_COM_MODE_IDS.AM)
            elseif ufcp_com2_modulation == UFCP_COM_MODE_IDS.FM then
                radio:set_modulation(UFCP_COM_MODE_IDS.FM)
            end
        end
...
local function ufcp_com2_net_date_validate(text, save)
    ...
    if text:len() >= ufcp_edit_lim or save then
        if text:len() >= ufcp_edit_lim and date_validation.is_valid(text) then
...
function update_com2()
    ...
    if ufcp_sel_format == UFCP_FORMAT_IDS.COM2 then
        ...
        -- [Fase R3.9] Publish current ECCM selector every UFCP frame so
        -- Systems/comsec.lua can pick up mode changes without polling.
        UFCP_COM2_ECCM:set(ufcp_com2_eccm)
