...
local function get_delta()
    -- INS position error (CEP) at the moment of the fix, in NM
    return AVIONICS_INS_ERR:get() / 1852.0
end
...
local function save()
    if ufcp_fix_lat ~= nil and ufcp_fix_lon ~= nil and ufcp_fix_elev ~= nil then
        -- Position update: correct the INS to the fix point (zero the drift error).
        AVIONICS_INS_FIX_REQ:set(1)
