...
local F5EM_FLIR_RT_MAX = 384
if get_plugin_option_value ~= nil then
  local ok, option_value = pcall(get_plugin_option_value, "F-5EM", "flirRtMax", F5EM_FLIR_RT_MAX)
  if ok and type(option_value) == "number" and option_value >= 256 and option_value <= 1024 then
    F5EM_FLIR_RT_MAX = option_value
  end
end
if LockOn_Options and LockOn_Options.cockpit then
  local cockpit_options = LockOn_Options.cockpit
  if cockpit_options.render_target_resolution == nil
      or cockpit_options.render_target_resolution > F5EM_FLIR_RT_MAX then
    cockpit_options.render_target_resolution = F5EM_FLIR_RT_MAX
  end
end
