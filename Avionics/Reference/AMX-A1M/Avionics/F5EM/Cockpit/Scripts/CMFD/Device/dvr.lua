...
function update_dvr()
    ...
    -- [Fase 6] No formato PLBCK, sobrescreve o text com tela de
    -- reproducao. DCS nao expoe o arquivo de track gravado para o
    -- aviao ler frame a frame -- piloto deve usar Track Replay do menu
    -- DCS. Aqui mostra status/contagem para feedback visual.
    if format == CMFD_DVR_FORMAT_IDS.PLBCK then
        text = "PLBCK MODE\n\n"
        text = text .. "USE DCS TRACK REPLAY\n"
        text = text .. "FROM MISSION MENU\n\n"
        text = text .. "RECORDED: " .. string.format("%03d", time_recorded / 60) .. " MIN\n"
        text = text .. "TOTAL   : " .. string.format("%03d", total_time_recorded / 60) .. " MIN"
    end
...
function SetCommandDvr(command,value, CMFD)
    if value == 1 then
        ...
        elseif (command==device_commands.CMFD1OSS23 or command==device_commands.CMFD2OSS23) then
            -- [Fase 6] Acessa formato PLBCK (playback). Alterna entre
            -- DVR (gravacao) e PLBCK (reproducao). Sem suporte a leitura
            -- real do MMC nesta camada -- a pagina PLBCK serve como
            -- placeholder informativo.
            if format == CMFD_DVR_FORMAT_IDS.DVR then
                format = CMFD_DVR_FORMAT_IDS.PLBCK
            else
                format = CMFD_DVR_FORMAT_IDS.DVR
            end
    ...
    -- [Fase 5.2] device_commands.UFCP_DVR agora chega ate aqui via
    -- forwarding explicito no cmfds.lua:SetCommand. Antes esse switch
    -- nao alcancava o state machine do DVR e ele ficava preso em
    -- "DVR OFF" mesmo com o switch UFCP em REC.
...
register_as_cmfd_item(SUB_PAGE_ID.DVR, nil, update_dvr, SetCommandDvr)
