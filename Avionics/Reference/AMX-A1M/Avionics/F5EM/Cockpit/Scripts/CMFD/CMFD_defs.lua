...
CMFD_STRINGDEFS_DEF_X08V  = {t_scale*0.80*FONT_W, 0.80*FONT_W, 0.01, 0.01, 0.01, 0.01}
...
function create_page_root()
    ...
    name = page_root.name
    page_root                 = CreateElement "ceMeshPoly"
    page_root.level           = CMFD_DEFAULT_LEVEL
    page_root.parent_element  = name
    page_root.isvisible       = false
    page_root.h_clip_relation = h_clip_relations.REWRITE_LEVEL -- COMPARE -- REWRITE_LEVEL
    page_root.vertices	      = {{-1, aspect}, {1, aspect}, {1, -aspect}, {1, -aspect}}
    page_root.indices		  = default_box_indices
    page_root.material        = CMFD_MATERIAL_PINK
    Add(page_root)
...
aspect = GetAspect()
default_stroke_thickness  = stroke_thickness
default_stroke_fuzziness  = stroke_fuzziness

function GetMainCenter()
    local area_height = aspect + 0.3
    return {0, area_height / 2 - 0.3}
end
