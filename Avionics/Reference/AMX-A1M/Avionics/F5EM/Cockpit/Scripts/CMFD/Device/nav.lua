...
local adhsi_nav_logic = dofile(LockOn_Options.script_path.."Systems/adhsi_nav_logic.lua")
...
local CMFD = {
    ...
    NAV_FYT_DTK_ETA_HOURS = get_param_handle("CMFD_NAV_FYT_DTK_ETA_HOURS"),
    NAV_FYT_DTK_ETA_MINS = get_param_handle("CMFD_NAV_FYT_DTK_ETA_MINS"),
    NAV_FYT_DTK_ETA_SECS = get_param_handle("CMFD_NAV_FYT_DTK_ETA_SECS"),
    NAV_FYT_DTK_STT_VALID = get_param_handle("CMFD_NAV_FYT_DTK_STT_VALID"),
...
local CMFD_NAV_DATA_TEXT = get_param_handle("CMFD_NAV_DATA_TEXT")
...
local function is_valid_waypoint(wpt)
    if not wpt or type(wpt.lat) ~= "number" or type(wpt.lon) ~= "number" then return false end
    return wpt.lat >= -90 and wpt.lat <= 90 and wpt.lon >= -180 and wpt.lon <= 180
end

local function get_valid_wpt_list()
    local wpt_index = {}
    for k=1,100 do
        if is_valid_waypoint(nav_fyt_list[k]) then
...
-- Own-ship position as the INS/EGI nav solution reports it: truth + the
-- simulated INS error (0 when not navigating, ~GPS CEP with GPS, growing on
-- pure INS). The F-5M nav computer steers from this, so a drifting INS biases
-- the steering and position readouts until a FIX corrects it.
function get_nav_self_pos()
    local lat_m, alt_m, lon_m = sensor_data.getSelfCoordinates()
    return lat_m + AVIONICS_INS_ERR_N:get(), alt_m, lon_m + AVIONICS_INS_ERR_E:get()
end

local function clear_nav_leg()
    CMFD.NAV_LEG_VALID:set(0)
    CMFD.NAV_LEG_COURSE:set(0)
    CMFD.NAV_LEG_XTRACK_M:set(0)
end

local function update_nav_leg()
    local selected_index = nav_fyt + 1
    local current_waypoint = nav_fyt_list[selected_index]
    local previous_waypoint = nav_fyt_list[selected_index - 1]
    local owns_route_leg = type(route) == "table"
        and nav_fyt >= 1
        and selected_index <= #route

    if not owns_route_leg or current_waypoint == nil or previous_waypoint == nil then
        clear_nav_leg()
        return
    end

    local own_north_m, _, own_east_m = get_nav_self_pos()
    local solution = adhsi_nav_logic.leg_solution(
        previous_waypoint.lat_m,
        previous_waypoint.lon_m,
        current_waypoint.lat_m,
        current_waypoint.lon_m,
        own_north_m,
        own_east_m
    )
    ...
    CMFD.NAV_LEG_VALID:set(1)
    CMFD.NAV_LEG_COURSE:set(solution.course_deg)
    CMFD.NAV_LEG_XTRACK_M:set(solution.cross_track_m)
end

local function calc_brg_dist_elev(dest_lat_m, dest_lon_m, dest_alt_m, orig_lat_m, orig_lon_m, orig_alt_m)
    local o_lat_m, o_alt_m, o_lon_m = get_nav_self_pos()
...
function update_nav()
    ...
    if nav_fyt_list[nav_fyt+1] then
        ...
        -- Auto-disable TIP after FYT overflight: once within 2 NM and now outbound,
        -- keep the cue from lingering on the previous leg.
        if UFCP_TIP_DIR:get() ~= 0
           and nav_fyt_last == nav_fyt
           and distance_last >= 0
           and distance < (2 / 0.000539957)
           and distance > distance_last then
            UFCP_TIP_DIR:set(0)
        end
        ...
        local eta = (get_absolute_model_time() + time) % (24 * 3600)
        local eta_secs = math.floor(eta % 60)
        local eta_mins = math.floor((eta / 60) % 60)
        local eta_hours = math.floor((eta / 3600) % 24)
        ...
        CMFD.NAV_FYT_DTK_ETA_HOURS:set(eta_hours)
        CMFD.NAV_FYT_DTK_ETA_MINS:set(eta_mins)
        CMFD.NAV_FYT_DTK_ETA_SECS:set(eta_secs)
        CMFD.NAV_FYT_DTK_STT_VALID:set(speed_to_target >= 0 and 1 or 0)
        ...
        CMFD.NAV_FYT_OAP_ELV:set(nav_fyt_list[nav_fyt+1].oap_elev)
        ...
        CMFD.NAV_FYT_DTK_ETA_HOURS:set(0)
        CMFD.NAV_FYT_DTK_ETA_MINS:set(0)
        CMFD.NAV_FYT_DTK_ETA_SECS:set(0)
        CMFD.NAV_FYT_DTK_STT_VALID:set(0)
    ...
    elseif CMFD_NAV_FORMAT:get() == CMFD_NAV_FORMAT_IDS.AC then
        local text =    ""
        local lat_m, alt_m, lon_m = get_nav_self_pos()
    ...
    elseif CMFD_NAV_FORMAT:get() == CMFD_NAV_FORMAT_IDS.DATA then
        local lat_m, alt_m, lon_m = sensor_data.getSelfCoordinates()
        local lat, lon = Terrain.convertMetersToLatLon(lat_m, lon_m)
        local velx, vely, velz = sensor_data.getSelfVelocity()   -- m/s  x=N  y=Up  z=E

        -- Measured (truth) side
        local tas   = sensor_data.getTrueAirSpeed() * 1.94384
        local mach  = sensor_data.getMachNumber()
        local vvel  = vely * 196.8504
        local roll  = math.deg(sensor_data.getRoll())
        local pitch = math.deg(sensor_data.getPitch())
        local hdg   = get_avionics_hdg()
        local baro  = sensor_data.getBarometricAltitude() * 3.28084

        -- INS (ESTIMADO) side = truth + simulated INS error (from the EGI drift model)
        local ins_valid = AVIONICS_INS_VALID:get() > 0.5
        local err_n   = AVIONICS_INS_ERR_N:get()
        local err_e   = AVIONICS_INS_ERR_E:get()
        local vn_err  = AVIONICS_INS_VN_ERR:get()
        local ve_err  = AVIONICS_INS_VE_ERR:get()
        local hdg_err = AVIONICS_INS_HDG_ERR:get()

        local cos_lat = math.cos(math.rad(lat))
        if cos_lat == 0 then cos_lat = 1e-6 end
        local ins_lat = lat + err_n / 111320.0
        local ins_lon = lon + err_e / (111320.0 * cos_lat)
        local ins_hdg = (hdg + hdg_err) % 360

        local function dm(v, ns)
            local h
            if ns then h = (v >= 0) and "N" or "S" else h = (v >= 0) and "E" or "W" end
            v = math.abs(v)
            local fmt = ns and "%s%02.0f`%05.2f'" or "%s%03.0f`%05.2f'"
            return string.format(fmt, h, math.floor(v), (v - math.floor(v)) * 60)
        end
        local function row(label, e, m)
            return string.format("%-9s%11s%11s\n", label, e, m)
        end
        local function kt(v) return string.format("%4.0fKT", v) end
        local NA = "----"

        local text = string.format("%-9s%11s%11s\n", "", "ESTIMADO", "MEDIDO")
        if ins_valid then
            text = text .. row("TAS",       kt(tas),                           kt(tas))
            text = text .. row("MACH",      string.format("%5.2f", mach),      string.format("%5.2f", mach))
            text = text .. row("VEL E",     kt((velz + ve_err) * 1.94384),     kt(velz * 1.94384))
            text = text .. row("VEL N",     kt((velx + vn_err) * 1.94384),     kt(velx * 1.94384))
            text = text .. row("VER VEL",   string.format("%5.0f", vvel),      string.format("%5.0f", vvel))
            text = text .. row("ROLL",      string.format("%4.0f`", roll),     string.format("%4.0f`", roll))
            text = text .. row("PITCH",     string.format("%4.0f`", pitch),    string.format("%4.0f`", pitch))
            text = text .. row("HDG",       string.format("%03.0f`", ins_hdg), string.format("%03.0f`", hdg))
            text = text .. row("P.P LAT",   dm(ins_lat, true),                 dm(lat, true))
            text = text .. row("P.P LON",   dm(ins_lon, false),                dm(lon, false))
            text = text .. row("PRESS ALT", string.format("%5.0fFT", baro),    string.format("%5.0fFT", baro))
            text = text .. row("BARO ALT",  string.format("%5.0fFT", baro),    string.format("%5.0fFT", baro))
        else
            text = text .. row("TAS",       NA, kt(tas))
            text = text .. row("MACH",      NA, string.format("%5.2f", mach))
            text = text .. row("VEL E",     NA, kt(velz * 1.94384))
            text = text .. row("VEL N",     NA, kt(velx * 1.94384))
            text = text .. row("VER VEL",   NA, string.format("%5.0f", vvel))
            text = text .. row("ROLL",      NA, string.format("%4.0f`", roll))
            text = text .. row("PITCH",     NA, string.format("%4.0f`", pitch))
            text = text .. row("HDG",       NA, string.format("%03.0f`", hdg))
            text = text .. row("P.P LAT",   NA, dm(lat, true))
            text = text .. row("P.P LON",   NA, dm(lon, false))
            text = text .. row("PRESS ALT", NA, string.format("%5.0fFT", baro))
            text = text .. row("BARO ALT",  NA, string.format("%5.0fFT", baro))
        end

        CMFD_NAV_DATA_TEXT:set(text)
        CMFD_NAV_PG_PREV:set(0)
        CMFD_NAV_PG_NEXT:set(0)
    ...
    elseif CMFD_NAV_FORMAT:get() == CMFD_NAV_FORMAT_IDS.AFLD then
        ...
        local line = ""
        for i=91, 100 do
            if is_valid_waypoint(nav_fyt_list[i]) then
    ...
    elseif CMFD_NAV_FORMAT:get() == CMFD_NAV_FORMAT_IDS.MARK then
        ...
        if not has_mark then
            text = text .. "SEM MKPT SALVO\n"
            text = text .. "USE UFCP MARK/ENTR\n"
        end
...
function SetCommandNav(command,value, CMFD)
    if (command==device_commands.CMFD1OSS2 or command==device_commands.CMFD2OSS2) and value == 1 then 
    ...
    elseif (command==device_commands.CMFD1OSS6 or command==device_commands.CMFD2OSS6) and value == 1 then
    ...
    elseif (command==device_commands.CMFD1OSS8 or command==device_commands.CMFD2OSS8) and value == 1 then
        nav_format = CMFD_NAV_FORMAT_IDS.DATA
...
register_as_cmfd_item(SUB_PAGE_ID.NAV, post_initialize_nav, update_nav, SetCommandNav)
