...
function SetCommandUfc(command,value, CMFD)
    local ufcp = GetDevice(devices.UFCP)
    if CMFD["Format"]:get() ~= SUB_PAGE_ID.UFC then return end
...
register_as_cmfd_item(SUB_PAGE_ID.UFC, nil, update_ufc, SetCommandUfc)