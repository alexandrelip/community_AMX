...
-- Static checklist text tables (F-5EM emergency + normal procedures).
local CL = dofile(LockOn_Options.script_path .. "Systems/checklists_f5em.lua")
...
-- Refresh the checklist body param from the checklists_f5em.lua tables.
-- Called on any submenu / level change so the CMFD text updates immediately.
local function refresh_text()
    CHECKLIST_TEXT:set(CL.get(submenu, level))
end

function update_emer()
    if level == 0 then
        -- Main page (menu splash handled by indicator's static Poly_Text).
    elseif level == 1 or level == 2 then
        -- Submenu / MORE page: body already published on transition; no per-frame work.
    else
        -- Reserved for future item pages (level 3).
    end
end
...
function SetCommandEmer(command,value, CMFD)
    if value == 1 then
        if level == 0 then
            -- Main page
...
register_as_cmfd_item(SUB_PAGE_ID.EMER, post_initialize_emer, update_emer, SetCommandEmer)