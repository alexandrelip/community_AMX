...
-- BUGFIX: page_root was gated on SUB_PAGE_ID.CHECKLIST which does not exist in
-- CMFD_pageID_defs.lua (only SUB_PAGE_ID.EMER). With nil the compare was
-- unreachable and the whole EMER page rendered EMPTY (menu OSBs hidden).
-- emer.lua registers on SUB_PAGE_ID.EMER (see its L102).
page_root.controllers = {{"parameter_compare_with_number",0,SUB_PAGE_ID.EMER}}
...
-- [Checklist body] Text block shown when a submenu is selected
-- (CMFD_CHECKLIST_LEVEL >= 1). Content published by CMFD/Device/emer.lua
-- as CMFD_CHECKLIST_TEXT from Systems/checklists_f5em.lua.
local cl_body          = CreateElement "ceStringPoly"
cl_body.material       = CMFD_FONT_DEF
cl_body.stringdefs     = CMFD_STRINGDEFS_DEF_X08
cl_body.init_pos       = {-0.40, 0.55}
cl_body.alignment      = "LeftTop"
cl_body.value          = ""
cl_body.formats        = {"%s"}
cl_body.element_params = {"CMFD"..tostring(CMFDNu).."_BRIGHT", "CMFD_CHECKLIST_TEXT", "CMFD_CHECKLIST_LEVEL"}
cl_body.controllers    = {
    {"opacity_using_parameter", 0},
    {"text_using_parameter", 1, 0},
    {"parameter_in_range", 2, 0.5, 3.5},   -- show only when LEVEL >= 1
}
cl_body.parent_element = page_root.name
AddElementObject(cl_body)
cl_body = nil