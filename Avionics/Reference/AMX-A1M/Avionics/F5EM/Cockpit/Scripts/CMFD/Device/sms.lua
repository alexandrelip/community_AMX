...
local SMS_ON = get_param_handle("SMS_ON")
...
local function clear_sj_selection()
    for i=1,7 do
        get_param_handle("WPN_SJ_STO"..tostring(i).."_SEL"):set(0)
    end
end
...
function update_sms()
    ...
    SMS_ON:set(get_cockpit_draw_argument_value(1844) > 0 and 1 or 0)
...
local function SetCommandSmsSj(command,value, CMFD)
    ...
    elseif command==device_commands.CMFD1OSS5 or command==device_commands.CMFD2OSS5 then
        weapons:performClickableAction(device_commands.WPN_SJ_RELEASE, 1, true)
...
local function SetCommandSmsAg(command,value, CMFD)
    ...
    if value ~= 1 or sms_inv == 1 then return 0 end
...
function SetCommandSms(command,value, CMFD)
    if SMS_ON:get() == 0 then return end
...
register_as_cmfd_item(SUB_PAGE_ID.SMS, post_initialize_sms, update_sms, SetCommandSms)